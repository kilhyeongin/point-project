...
import FavoritePartnerButton from "@/app/customer/FavoritePartnerButton";
    ...
    <article className="bg-card rounded-2xl overflow-hidden flex flex-col shadow-card hover:shadow-card-hover transition-all duration-200">
      ...
      <div className="p-4 flex flex-col gap-3 flex-1">
        ...
        <div className="mt-auto pt-1">
          <Link href={`/customer/partner/${item.id}`} className={cn(buttonVariants({}), "w-full h-[45px]")}>
            상세 보기
          </Link>
        </div>
      </div>
    ...
    <article className="bg-card rounded-2xl overflow-hidden flex flex-col shadow-card hover:shadow-card-hover transition-all duration-200">
      ...
      <div className="p-3 flex flex-col gap-1.5 flex-1">
        ...
        <div className="mt-auto pt-2 flex flex-col gap-1.5">
          ...
          <Link href={`/customer/partner/${item.id}`} className={cn(buttonVariants({ size: "sm" }), "w-full h-10")}>
            상세 보기
          </Link>
        </div>
      </div>
    ...
    >
      <div className="space-y-4">
        ...
        {loading ? (
          ...
          <FullCard item={items[0]} onFavoriteChanged={handleFavoriteChanged} />
        ) : (
          <div className="grid grid-cols-2 gap-3">
            {items.map((item) => (
              <GridCard key={item.id} item={item} onFavoriteChanged={handleFavoriteChanged} />
            ))}
          </div>
        )}
      </div>
