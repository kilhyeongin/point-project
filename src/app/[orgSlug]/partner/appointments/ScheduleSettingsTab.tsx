  ...
  return (
    <div className="space-y-4">
        ...
        <label className="flex items-center gap-3 cursor-pointer select-none w-fit">
          <div className="relative">
            ...
            <div className={cn(
              ...
              schedule.closedOnHolidays ? "bg-foreground border-foreground" : "bg-background border-border"
            )}>
      ...
      <section className="bg-card shadow-card rounded-2xl p-5 space-y-4">
        ...
        <p className="text-xs text-muted-foreground">점심시간 등 예약을 받지 않을 시간대를 설정합니다.</p>
        {schedule.breakStart !== "" && (
          <div className="flex items-end gap-2 flex-wrap">
            <div className="space-y-1.5">
              <label className="block text-xs font-bold text-muted-foreground">휴무 시작</label>
              <Input type="time" value={schedule.breakStart} onChange={(e) => setSchedule((p) => ({ ...p, breakStart: e.target.value }))} className="w-36 h-11" />
            </div>
            <span className="pb-2.5 text-muted-foreground font-bold text-base">~</span>
            <div className="space-y-1.5">
              <label className="block text-xs font-bold text-muted-foreground">휴무 종료</label>
              <Input type="time" value={schedule.breakEnd} onChange={(e) => setSchedule((p) => ({ ...p, breakEnd: e.target.value }))} className="w-36 h-11" />
            </div>
          </div>
        )}
      </section>
      ...
      <section className="bg-card shadow-card rounded-2xl p-5 space-y-4">
        ...
        {slots.length > 0 && (
          <div>
            <p className="text-xs font-bold text-muted-foreground mb-2">슬롯 미리보기 ({slots.length}개)</p>
            <div className="flex flex-wrap gap-1.5">
              {slots.slice(0, 24).map((s) => (
                <span key={s} className="px-2.5 py-1 rounded-lg bg-muted text-xs font-bold text-foreground">{s}</span>
              ))}
              {slots.length > 24 && (
                <span className="px-2.5 py-1 rounded-lg bg-muted text-xs text-muted-foreground">+{slots.length - 24}개</span>
              )}
            </div>
          </div>
        )}
      ...
      <section className="bg-card shadow-card rounded-2xl p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            ...
            <Input type="number" min={1} max={99} value={schedule.maxPerSlot} onChange={(e) => setSchedule((p) => ({ ...p, maxPerSlot: Number(e.target.value) || 1 }))} className="w-24 h-10" />
          </div>
          <div className="space-y-1.5">
            ...
            <Input type="number" min={1} max={365} value={schedule.advanceDays} onChange={(e) => setSchedule((p) => ({ ...p, advanceDays: Number(e.target.value) || 30 }))} className="w-24 h-10" />
          </div>
        </div>
      </section>
      ...
      <section className="bg-card shadow-card rounded-2xl p-5 space-y-4">
        ...
        <div className="flex items-end gap-2">
          <div className="space-y-1.5 flex-1 max-w-[180px]">
            ...
            <Input type="date" id="blockedDateInput" min={new Date().toISOString().slice(0, 10)} className="h-11" />
          </div>
          <button
            type="button"
            onClick={() => {
              ...
              if (!val || schedule.blockedDates.includes(val)) return;
      ...
      <section className="bg-card shadow-card rounded-2xl p-5">
        <div className="flex items-center gap-4 flex-wrap">
          ...
          {msg && (
            <p className={`text-sm font-bold ${isError ? "text-destructive" : "text-emerald-600"}`}>{msg}</p>
          )}
        </div>
      </section>
    </div>
