...
type Appointment = {
  ...
  staffMemo: string;
  statusHistory: { status: string; label: string; note: string; at: string | null }[];
};
...
function StatGrid({ items }: { items: { label: string; value: number; color: string; desc: string }[] }) {
  ...
  useEffect(() => {
    ...
    function handleClick(e: MouseEvent) {
      ...
      if (!target.closest("[data-stat-card]")) setOpenIdx(null);
    }
...
export default function PartnerAppointmentsPage() {
  ...
  function handleMemoSaved(id: string, memo: string) {
    setItems((prev) => prev.map((i) => i.id === id ? { ...i, staffMemo: memo } : i));
    setSelected((prev) => prev?.id === id ? { ...prev, staffMemo: memo } : prev);
  }
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-1.5 text-muted-foreground">
                        ...
                        <span className="text-sm font-bold text-foreground">{formatDate(item.appointmentAt)}</span>
                      </div>
                      {item.customerPhone && (
                      ...
                      <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                        ...
                        <span>신청 {formatDateShort(item.appliedAt)}</span>
                      </div>
                      {item.confirmedAt && (
                        <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                          <Clock className="w-3 h-3 shrink-0" />
                          <span>확정 {formatDateShort(item.confirmedAt)}</span>
                        </div>
                      )}
                      {item.cancelledAt && (
                        <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                          <Clock className="w-3 h-3 shrink-0" />
                          <span>취소 {formatDateShort(item.cancelledAt)}</span>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border bg-muted/30">
                      ...
                      <th className="px-4 py-3 text-left text-xs font-black text-muted-foreground whitespace-nowrap">확정일시</th>
                      <th className="px-4 py-3 text-left text-xs font-black text-muted-foreground whitespace-nowrap">취소일시</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filtered.map((item) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3.5 text-muted-foreground whitespace-nowrap">{item.confirmedAt ? formatDateShort(item.confirmedAt) : "-"}</td>
                        <td className="px-4 py-3.5 text-muted-foreground whitespace-nowrap">{item.cancelledAt ? formatDateShort(item.cancelledAt) : "-"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {selected && (
        <>
          ...
          >
            ...
            <PanelBody selected={selected} onMemoSaved={handleMemoSaved} />
...
function PanelBody({ selected, onMemoSaved }: { selected: Appointment; onMemoSaved: (id: string, memo: string) => void }) {
  const [memo, setMemo] = useState(selected.staffMemo ?? "");
  const [memoSaving, setMemoSaving] = useState(false);

  useEffect(() => {
    setMemo(selected.staffMemo ?? "");
  }, [selected.id]);

  async function saveMemo() {
    setMemoSaving(true);
    try {
      await fetch(`/api/partner/appointments/${selected.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ staffMemo: memo }),
      });
      onMemoSaved(selected.id, memo);
    } finally {
      setMemoSaving(false);
    }
  }
    ...
    <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4 sm:space-y-5">
      ...
      <div className="bg-muted/30 rounded-xl p-3.5 space-y-2.5">
        ...
        {selected.customerPhone && (
          <a href={`tel:${selected.customerPhone}`} className="flex items-center gap-2 text-sm font-semibold text-foreground hover:text-primary transition-colors">
      ...
      <div>
        ...
        {selected.appointmentNote && (
          <div className="mt-3 flex items-start gap-2.5">
            ...
            <div>
              <div className="text-[10px] text-muted-foreground mb-0.5">요청사항</div>
              <div className="text-sm text-foreground leading-relaxed">{selected.appointmentNote}</div>
            </div>
          </div>
        )}
      </div>
      ...
      {/* 직원 메모 */}
      <div>
        <p className="text-xs font-black text-muted-foreground uppercase tracking-wide mb-2">직원메모</p>
        <textarea
          value={memo}
          onChange={(e) => setMemo(e.target.value)}
          maxLength={500}
          placeholder={"내부 직원용으로 고객에게 보이지 않는 메모입니다.\n(최대 500자 입력가능)"}
          className="w-full text-sm p-3 rounded-xl border border-input bg-background resize-none min-h-[90px] focus:outline-none focus:ring-2 focus:ring-ring leading-relaxed placeholder:text-muted-foreground/50"
        />
        <div className="flex items-center justify-between mt-1.5">
          <span className="text-xs text-muted-foreground">{memo.length} / 500자</span>
          <button
            type="button"
            onClick={saveMemo}
            disabled={memoSaving}
            className="h-7 px-3 rounded-lg bg-foreground text-background text-xs font-bold disabled:opacity-50 hover:opacity-80 transition-opacity"
          >
            {memoSaving ? "저장 중..." : "저장"}
          </button>
        </div>
      ...
      {/* 진행이력 */}
      <div>
        <p className="text-xs font-black text-muted-foreground uppercase tracking-wide mb-3">진행이력</p>
        {selected.statusHistory.length === 0 && !selected.appliedAt ? (
          ...
          <div className="space-y-3">
            {selected.statusHistory.map((h, i) => (
              <div key={i} className="flex gap-3 text-sm">
                ...
                <div className="pb-3">
                  <div className="font-bold text-foreground">{h.label}</div>
                  <div className="text-xs text-muted-foreground mt-0.5">{h.at ? formatDate(h.at) : "-"}</div>
                  {h.note && <div className="text-xs text-primary mt-0.5">{h.note}</div>}
                </div>
              </div>
            ))}
            {/* 신청은 항상 마지막에 고정 */}
            {selected.appliedAt && (
              <div className="flex gap-3 text-sm">
                ...
                <div>
                  <div className="font-bold text-foreground">신청</div>
                  <div className="text-xs text-muted-foreground mt-0.5">{formatDate(selected.appliedAt)}</div>
                </div>
              </div>
            )}
          </div>
