                ...
                <>
                  <div className="space-y-2">
                    {reqItems.slice(0, visibleReq).map((it) => (
                      <div key={it.id} className="flex items-center justify-between gap-3 py-2.5 border-b border-border/60 last:border-0">
                        <div className="min-w-0">
                          <p className="text-sm font-black text-foreground truncate">{it.to?.name}{(socialLabel(it.to?.socialProvider) ?? it.to?.username) ? <span className="font-normal text-muted-foreground"> ({socialLabel(it.to?.socialProvider) ?? it.to?.username})</span> : null}</p>
                          <p className="text-xs text-muted-foreground mt-0.5">{formatDateText(it.createdAt)}</p>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                ...
                <>
                  <div className="space-y-2">
                    {historyItems.slice(0, visibleHistory).map((it) => (
                      <div key={it.id} className="flex items-center justify-between gap-3 py-2.5 border-b border-border/60 last:border-0">
                        <div className="min-w-0">
                          <p className="text-sm font-black text-foreground truncate">{it.customer?.name}{(socialLabel(it.customer?.socialProvider) ?? it.customer?.username) ? <span className="font-normal text-muted-foreground"> ({socialLabel(it.customer?.socialProvider) ?? it.customer?.username})</span> : null}</p>
                          <p className="text-xs text-muted-foreground mt-0.5">{formatDateText(it.createdAt)}</p>
                          {it.note && <p className="text-xs text-muted-foreground truncate">메모: {it.note}</p>}
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
