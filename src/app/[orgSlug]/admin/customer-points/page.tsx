...
export default function CustomerPointsPage() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      setSearching(true);
      try {
        const res = await fetch(`/api/admin/accounts?role=CUSTOMER&q=${encodeURIComponent(userQ)}&limit=10`);
  ...
  const loadLedger = useCallback(async (p = 1) => {
    ...
    try {
      const params = new URLSearchParams({ role: "CUSTOMER", page: String(p) });
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!selectedUser) { toast.error("고객을 선택해주세요."); return; }
    ...
    setSubmitting(true);
    try {
      const finalAmount = mode === "charge" ? amtNum : -amtNum;
      const res = await fetch("/api/admin/adjust", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ targetUserId: selectedUser.id, amount: finalAmount, note: note || (mode === "charge" ? "관리자 포인트 지급" : "관리자 포인트 차감") }),
      });
      const data = await res.json();
      if (!data.ok) { toast.error(data.message ?? "처리 실패"); return; }
      toast.success(`${selectedUser.name} ${mode === "charge" ? `${fmt(amtNum)}P 지급` : `${fmt(amtNum)}P 차감`} 완료`);
  ...
  return (
    <main className="max-w-3xl space-y-5">
      ...
      <div>
        ...
        <h1 className="text-2xl font-black text-foreground tracking-tight">고객 포인트 관리</h1>
        <p className="text-sm text-muted-foreground mt-1">고객 포인트를 지급하거나 차감합니다.</p>
      </div>
      ...
      <div className="bg-card border border-border rounded-2xl p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* 고객 검색 */}
          <div>
            <label className="block text-sm font-bold text-foreground mb-1.5">고객 선택</label>
            {selectedUser ? (
          ...
          >
            {submitting ? <span className="flex items-center justify-center gap-2"><Loader2 className="w-4 h-4 animate-spin" />처리 중</span> : mode === "charge" ? "지급하기" : "차감하기"}
          </button>
        </form>
      </div>
      ...
      <div className="bg-card border border-border rounded-2xl p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-black text-foreground">고객 전체 내역</h2>
