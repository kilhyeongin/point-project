      ...
      {tab === "profile" || tab === "settings" ? (
        <div className="bg-card shadow-card rounded-2xl mt-4 overflow-hidden">
          ...
          <div className="flex border-b border-border px-2">
            {([["profile", "내 정보"], ["settings", "설정"]] as ["profile" | "settings", string][]).map(([t, label]) => (
              <button
                key={t}
                type="button"
                onClick={() => setTab(t)}
              ...
              <div className="border-t border-border/60 px-5 py-4 space-y-4">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <textarea value={form.intro} onChange={(e) => update("intro", e.target.value)} placeholder="고객에게 보여줄 업체 소개글" maxLength={2000}
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">{form.intro.length} / 2000자</p>
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-foreground mb-1.5">혜택 설명</label>
                    <textarea value={form.benefitText} onChange={(e) => update("benefitText", e.target.value)} placeholder="예: 방문 상담 시 5,000P 사용 가능" maxLength={2000}
                      className="w-full p-3 rounded-xl border border-input bg-background text-sm leading-relaxed resize-none min-h-[140px] focus:outline-none focus:ring-2 focus:ring-ring" />
                    <p className="mt-1 text-xs text-muted-foreground">{form.benefitText.length} / 2000자</p>
                  </div>
                </div>
              </div>
              ...
              <div className="border-t border-border/60 px-5 py-4 space-y-4">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Input value={form.applyUrl} onChange={(e) => update("applyUrl", e.target.value)} placeholder="https://example.com/apply" />
                  </div>
                  <div>
