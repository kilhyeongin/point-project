...
export async function DELETE(_req: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    if (!doc) {
      const exists = await WithdrawalRequest.findOne({ _id: id, organizationId: orgId, partnerId }, { status: 1 }).lean();
    ...
    console.error("[PARTNER_WITHDRAWAL_DELETE_ERROR]", error);
    return NextResponse.json({ ok: false, message: "출금 취소 중 오류가 발생했습니다." }, { status: 500 });
  }
}
